...
const NewsCards = ({catData}) => {
  ...
  return (
    <div>
        ...
        {
        catData.data.length > 0 ? catData.data.map(data => <div className="card bg-base-100 border border-gray-200 rounded-md mb-8 overflow-hidden">
            {/* Header: Author Info */}
            <div className="flex items-center justify-between p-3 bg-gray-100">
                <div className="flex items-center gap-3">
                {/* Static Profile Image */}
                <Image className='rounded-full' src={data.author.img} alt="NewsRepoterImg" width={40} height={40}/>
                <div>
                    <p className="font-semibold text-[#403F3F] text-sm">{data.author.name}</p>
            ...
            <div className="px-4 py-4">
                    ...
                    {/*News Image */}
                    <Image className='w-full h-full object-cover' src={data.image_url} alt="thumbnail_url" width={80} height={80}/>

                    
                    <p className="text-[#706F6F] text-sm leading-6 mt-6  line-clamp-4">{data.details}</p>
                    <Link href='/'>
            ...
            <div className="flex items-center justify-between p-4">
                    ...
                    <span className="text-[#706F6F] font-medium text-sm">{data.rating.number}</span>
                    </div>
